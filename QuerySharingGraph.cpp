...
//    // Step 1: Initialize
//    for (const auto& q : Q) {
//        Psi.addEdge({q.s, 0, static_cast<int> (ceil(static_cast<double>(q.k)/2))}, q);
//        ME[q.s].insert({q.s, 0, static_cast<int> (ceil(static_cast<double>(q.k)/2))});
//        que.push(q.s);
//    }

//    int kmax = 0;
//    for (const auto& q : Q) {
//        kmax = std::max(kmax, static_cast<int>(ceil(static_cast<double>(q.k)/2)));
//    }
...
void QuerySharingGraph::DetectCommonQuery_r(DirectedGraph& G, QueryGroup& Q, QuerySharingGraph& Psi) {
   ...
   // Step 1: Initialize
   for (const auto& q : Q.queries) {
       Psi.addEdge({q.s, 0, static_cast<int> (floor(static_cast<double>(q.k)/2))}, q);
       ME[q.s].insert({q.s, 0, static_cast<int> (floor(static_cast<double>(q.k)/2))});
       que.push(q.s);
       // std::cout << "dw" << q.s << std::endl;
   }
   ...
   int kmax = 0;
   for (const auto& q : Q.queries) {
       kmax = std::max(kmax, static_cast<int>(floor(static_cast<double>(q.k)/2)));
   }
